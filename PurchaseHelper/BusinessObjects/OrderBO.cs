...
namespace PurchaseHelper.BusinessObjects
{
    public class OrderBO : BOBase<OrderModel>
    {
        ...
        public override OrderModel GetByID(int id)
        {
            ...
            if (order.MerchandiseID > 0)
            {
                MerchandiseBO merchandise = new MerchandiseBO(_connString);
                order.Merchandise = merchandise.GetByID(order.MerchandiseID);
        ...
        public decimal CalculateProfit(OrderModel order)
        {
            decimal profit = 0;
            decimal cost = 0;
            decimal exchangeRate = order.ExchangeRate.HasValue ? order.ExchangeRate.Value : order.RealTimeExchangeRate;
            cost = order.PurchasePrice.HasValue ? order.PurchasePrice.Value : order.Merchandise != null ? order.Merchandise.USDPrice * (1 + order.Merchandise.Tax / 100) : 0;
            cost = cost * exchangeRate * order.Number;
            ...
            if (order.ShippingCost.HasValue)
                cost += order.ShippingCost.Value * exchangeRate;
